...
const CONTRACT_ADDRESS="0x28fBab7EE35D72C7c654D6931ce1b108FEf83DE2"//mintReward
const GameFTs_ABI = [
    {
      "inputs": [],
...
const CONTRACT_ADDRESS3="0x9D3acD8efD67078bfDFdB82BD5b9aaE863Aa13Ff";//mintItem
const WEAPONS_ABI=[
    {
      "inputs": [
        {
...
const BDAG_ADDRESS="0x12CBa3319D391e97932E10E97aE82B3603aB2Ffa";
const ERC20_ABI = [
    {
      "inputs": [
        {
...
const ALLOY_ADDRESS="0x2be9a62b05f29b645Ad55a665339b841D5bE35a7"//mintCompoundNFT
const ALLOY_ABI=[       {
      "inputs": [],
      "stateMutability": "nonpayable",
      "type": "constructor"
...
this.load.image('rare_crown', "../assetsFT/15.png");
  ...
  this.load.spritesheet('npc1','../assets/shopkeeper.png',{
    ...
    this.load.image('enemy','../assets/enemy_worm.png');
...
this.addToInventory("ROCK_SHEILD","'../assetsWeapon/11.png'",'Basic defence weapon',1);
 ...
 this.elementColors2 = {
  ...
  Bi: "808090",
  Sb: "808090",
  La: "808090", // rare earth
Ga: "B0C4DE", // light steel blue
In: "B0C4DE", // light steel blue
Tl: "B0C4DE",
Ge: "DAA520", // same as Si
Sn: "A9A9A9", // dark gray
Pb: "A9A9A9",
As: "9ACD32", // yellow-green
};
...
async handleInsideZone() {
  ...
  if (x == 1) {
    this.msg = this.createText(this, "You found an alloy, prepare for battle", true);
    this.bgm.stop();
    this.startBattle();
    this.events.once('battle-ended', async (data) => {
      this.bgm.play();
      this.battleResult = data.result === 'win';
      console.log("Battle result:", this.battleResult ? "Win" : "Lose");
      if (this.msg) {
        this.msg.text.destroy();
        this.msg.overlay.destroy();
      }
      if (this.battleResult === true) {
        ...
        this.msg = this.createText(this, "You lost the battle, try again", true);
      }
    });
...
async handleOUtsideZone(scene){
  ...
  await AssetManager.AssetHolder("Bone","Bone","../assetsFT/14.png","A bone from ancient times",14,1);
  //this.playerAssets=await AssetManager.loadPlayerAssets();
  console.log("WorldScene - playerAssets:", this.playerAssets);
...
categorizeWeapons(playerAssets) {
    const categories = {
        pickaxe: [],
        sword: [],
        shield: [],
    ...
    if (!playerAssets || typeof playerAssets !== 'object') {
        console.warn('categorizeWeapons: playerAssets is missing or invalid:', playerAssets);
        return categories;
    }

    Object.values(playerAssets).forEach(weapon => {
        if (!weapon || !weapon.name) return;

        const name = weapon.name.toLowerCase();
        if (name.includes("pickaxe")) categories.pickaxe.push(weapon);
        else if (name.includes("sword")) categories.sword.push(weapon);
        else if (name.includes("shield") || name.includes("sheild")) categories.shield.push(weapon);
        else if (name.includes("potion")) categories.potion.push(weapon);
    });

    return categories;
}
showWeaponSelection() {
    const playerWeapons = this.categorizeWeapons(this.playerAssets);

    if (this.selectionPanel) return; // Prevent multiple panels
    ...
    panelHTML.innerHTML = `
        ...
        <div id="selection-panel">
            ...
            <button id="confirm-weapons">Confirm</button>
        </div>
    `;

    document.body.appendChild(panelHTML);
    this.selectionPanel = panelHTML;

    const rowsContainer = panelHTML.querySelector('#weapon-rows');
    const confirmBtn = panelHTML.querySelector('#confirm-weapons');

    const types = ['pickaxe', 'sword', 'shield', 'potion'];
    const selected = this.selectedWeapons || {};

    types.forEach(type => {
        const row = document.createElement('div');
        row.className = 'weapon-row';
        const weaponsOfType = playerWeapons[type] || [];

        if (weaponsOfType.length === 0) {
            // Add a blank box if no weapons of this type are available
            const blankBox = document.createElement('div');
            blankBox.className = 'weapon-option';
            blankBox.textContent = 'N/A'; // Optional: Add "N/A" text
            row.appendChild(blankBox);
        } else {
            weaponsOfType
                ...
                .forEach(weapon => {
                    ...
                    btn.dataset.tokenId = weapon.tokenId;
                    ...
                    btn.addEventListener('click', () => {
                        // Deselect all of the same type
                        row.querySelectorAll('.weapon-option').forEach(b => b.classList.remove('selected'));
    ...
    confirmBtn.addEventListener('click', () => {
        const missingSelections = types.filter(type => playerWeapons[type]?.length > 0 && !selected[type]);

        if (missingSelections.length > 0) {
            alert(`Please select a weapon for: ${missingSelections.join(', ')}`);
        } else {
            localStorage.setItem('selectedWeapons', JSON.stringify(selected));
            document.body.removeChild(this.selectionPanel);
            this.selectionPanel = null;
            console.log('Selected weapons:', selected);
        }
    });
}
startBattle() {
    const categorizedAssets = this.categorizeWeapons(this.playerAssets);

    // Wrap each selected weapon into an array so BattleScene can read it
    let weaponsToUse = {};

    if (this.selectedWeapons && Object.keys(this.selectedWeapons).length) {
        weaponsToUse = Object.fromEntries(
            Object.entries(this.selectedWeapons).map(([type, weapon]) => [type, [weapon]])
        );
    } else {
        // fallback: use most recently updated weapon of each type
        ['pickaxe', 'sword', 'shield', 'potion'].forEach(type => {
            const weapons = categorizedAssets[type];
            if (weapons && weapons.length > 0) {
                weaponsToUse[type] = [weapons.sort((a, b) => b.lastUpdated - a.lastUpdated)[0]];
            }
        });
    }

    console.log("Final weapons passed to battle:", weaponsToUse);
    ...
    this.scene.launch('BattleScene', {
        ...
        playerAssets: this.playerAssets,
    });
}
...
resume(data) {
    ...
    if (data && data.result) {
        ...
        // you can reset game, respawn enemy, etc.
    }
}
createText(scene, _text, _visible = true) {
 ...
 handleInventoryNavigation(time) {
    ...
    if (Phaser.Input.Keyboard.JustDown(this.keys.LEFT) && time - this.lastMoveTime > this.moveDelay) {
      ...
      this.menusound.play();
      this.inventoryUI.moveSelection(-6); 
      this.lastMoveTime = time;
    }
...
async loadCompoundImage(scene, e1, e2, mf, c1, c2) {
    ...
    this.textures.once('addtexture-' + key, () => {
        this.img = this.add.image(
            Math.floor(cam.centerX),  
            Math.floor(cam.centerY - 20),
            key
        );
        
        this.img.setOrigin(0.5, 0.5);
        this.img.setDisplaySize(30, 30);  
        this.img.setDepth(1000);
        this.img.setScrollFactor(0);
...
async function mintReward(itemName) {
  ...
  try {
    ...
    console.log("Elements:", elem1, elem2, "Fraction:", frac);
  ...
  Bi: "808090",
  Sb: "808090",
  Tc: "808090", // assume transition metal
La: "808090", // rare earth
Ga: "B0C4DE", // light steel blue
In: "B0C4DE", // light steel blue
Tl: "B0C4DE",
...
async  mintBDAGTokens(toAddress, amountBDAG) {
  try {
    const provider = new ethers.BrowserProvider(window.ethereum);
    const signer = await provider.getSigner();

    const bdagContract = new ethers.Contract(
      BDAG_ADDRESS, // Replace with your deployed BDAG token address
      [
    ...
    // Convert BDAG amount to 18-decimal wei (e.g., 1 BDAG = 1e18)
    const amountInWei = ethers.parseUnits(amountBDAG.toString(), 18);

    // Call the mintTo function
    const tx = await bdagContract.mintTo(toAddress, amountInWei,{gasLimit: 8000000}); // Adjust gas limit as needed
    console.log("Mint transaction sent:", tx.hash);

    await tx.wait();
    console.log(`✅ Successfully minted ${amountBDAG} BDAG to ${toAddress}`);

    // Fetch recipient's updated balance
    const recipientBalance = await bdagContract.balanceOf(toAddress);
    console.log("Recipient's updated balance:", ethers.formatUnits(recipientBalance, 18));
  } catch (err) {
    console.error("❌ BDAG minting failed:", err.message || err);
  }
}
...
try {
  ...
  console.error("Error calling getMintCost:", error);
    ...
    if(balance==0){
          this.mintBDAGTokens(walletAddress, 10.0);

    }
    console.log("BDAG balance after transfer:", ethers.formatUnits(balance));
    console.log("Total cost:", ethers.formatUnits(totalCost));
    console.log("BDAG balance:", balance.toString());
